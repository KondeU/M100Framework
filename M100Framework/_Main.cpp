...
// Remove external CLogger class and implement it locally.
...
public:
	...
	{
		string szFout;
		szFout = szPath + szLogFile + ".log";
		fout.open(szFout, ios::app);
	}
	...
	template<typename T> inline friend CLogger & operator<<(CLogger & cLogger, const T & t)
	{
		cout << t;
		cLogger.fout << t;
		return cLogger;
	...
	inline CLogger & operator<<(CLogger & (*pFunc)(CLogger&))
	{
		return pFunc(*this);
	}
...
int main(int argc, char *argv[])
{
	...
	if (0 != geteuid())
	{
		cLog << framework << "Program exited. Please run this execute as root!\n" << endl;
		return ERR_NOROOT;
	}
	...
	else
	{
		...
		cLog << framework << "Program exited. Target version is Unknown!\n" << endl;
		return ERR_UNKVER;
	}
	/*if (cLog.bFlightMove)
	{
	...
	{
		cLog << framework << "Program exited. Startup function return error.\n" << endl;
		return ERR_STARTUP;
	}
	...
	{
		cLog << framework << "Program exited. Cleanup function return error.\n" << endl;
		return ERR_CLEANUP;
	}

	cLog << framework << "Program exited normally.\n" << endl;
	return 0;
}
